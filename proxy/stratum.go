...
func (cs *Session) handleTCPMessage(s *ProxyServer, req *StratumReq) error {
	...
	case "eth_submitWork":
		...
		if !validShare {
			// A nonce can be well-formed but still below the configured RandomX
			// share difficulty. Returning a normal false result avoids treating
			// every below-target share as a protocol error/disconnect when the
			// operator raises proxy.randomx.shareDifficulty.
			log.Printf("Rejected eth_submitWork share from %s: below configured share difficulty", cs.ip)
