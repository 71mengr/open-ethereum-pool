...
func startProxy() {
        ...
        if cfg.Proxy.RandomX.Enabled {
                log.Printf("RandomX mining is ENABLED - epoch length: %d, cache size: %dMB, dataset: %dGB",
                        cfg.Proxy.RandomX.EpochLength,
                        cfg.Proxy.RandomX.CacheSizeMB,
                        cfg.Proxy.RandomX.DatasetSizeGB)
        } else {
                log.Printf("RandomX mining is DISABLED - using Ethash")
        }
